...
class CompatibilityLayer(nn.Module):
    ...
    def estimateH(self, adj, y, inputs=None, sample_mask=None):
        ...
        # ], axis=0)
        H = torch.concat([torch.mean(torch.gather(nodeH, torch.where(y[:, i]), axis=0), axis=0) for i in range(y.shape[1])], axis=0)
...
class LinBP(nn.Module):
    ...
    def forward(self, inputs, adj, y_train, train_mask):
        prior_belif = F.softmax(input, dim=1) # eq 4
...
class CPGNN(nn.Module):
    def __init__(self, num_features, hidden, out_dim, dropout):
        ...
        self.belief_estimator = MLP(num_features, hidden, out_dim, dropout)

    def forward(self, ipnuts, adj):
